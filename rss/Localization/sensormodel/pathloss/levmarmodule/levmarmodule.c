...
#include "./levmar-2.6/Axb.c"
#include "./levmar-2.6/compiler.h"
#include "./levmar-2.6/levmar.h"
#include "./levmar-2.6/lm.c"
#include "./levmar-2.6/misc.c"
...
/* Python wraper for levmar function to fit model z = p0 -
 * p1log((x-p2)^2+(y-p3)^2) */
// FILE *f;
// FILE *f2;
...
typedef struct xtradata {
  ...
  double y[max_size];
  double
      x2y2[max_size];  // x^2+y^2 computed to optimize distance to ap evaluation
...
double sigmoid(double x) { return 1 / (1 + exp(-sigmoidk * x)); }
...
void distance_squared(double *d2, double *p, void *data) {
  ...
  /* d2 = x^2-2*x2*p2 + p2^2 + y^2-2*y2*p3 + p3^2 + eps
     ...
     2*p2, 2*p3 and p2^2+p3^2 computed once and stored in twop2, twop3 and
     p22p32
 */
  ...
  for (i = 0; i < n; ++i) {
    d2[i] = dat->x2y2[i] + p22p32 - dat->x[i] * twop2 -
            dat->y[i] * twop3;  // god damn...
  }
}
...
void jacmodelfunp23(double *p, double *jac, int m, int n, void *data) {
  ...
  for (i = j = 0; i < n; ++i) {
    ...
    if (sign(dat->z[i][ap]) == 1) {
      ...
      zp = p[0] - p[1] * log(d2[i]);
      sigmoid_zp = sigmoid(zp);
      cte = 2 * (p[1] / d2[2]) * sigmoid_zp *
            (sigmoidk * (1 - sigmoid_zp) * (zp - dat->z[i][ap]) + 1);
    }
...
void jacmodelfunp01(double *p, double *jac, int m, int n, void *data) {
  ...
  for (i = j = 0; i < n; ++i) {
    ...
    if (sign(dat->z[i][ap]) == 1) {
      ...
      sigmoid_zp = sigmoid(zp);
      cte =
          sigmoid_zp * (sigmoidk * (1 - sigmoid_zp) * (zp - dat->z[i][ap]) + 1);
    }
    jac[j++] = cte;
    jac[j++] = -cte * ld2;
    jac[j++] = 0.0;
...
void init_ap(double *p, void *data) {
  ...
  if (debug) {
    fprintf(stderr, "Weighted xy:              \t %6.2f %6.2f\n", xavg, yavg);
  }
  xzc = xzc / nzeros;
  yzc = yzc / nzeros;
  if (debug) {
    fprintf(stderr, "Zeros c:                  \t %6.2f %6.2f\n", xzc, yzc);
  }
  ...
  if (debug) {
    fprintf(stderr, "Weighted avg xy [sum of squares] \t %6.2f %6.2f [%12.9f]\n", xavg,
           yavg, teavg);
  }
  ...
  if (debug) {
    fprintf(stderr, "Selected xy              \t %6.2f %6.2f\n", p[2], p[3]);
  }
  ...
  if (debug) {
    ...
    fprintf(stderr, "Jac_p0 (Sum of squares) %10.7f\n", teavg);
    fprintf(stderr, "Jac_p1 (Sum of squares) %10.7f\n", temax);
    teavg = 0;
    ...
    fprintf(stderr, "Jac_xap (Sum of squares) %10.7f\n", teavg);
    fprintf(stderr, "Jac_yap (Sum of squares) %10.7f\n", temax);
  }
  // PyMem_Free(e);
}
...
int load_variables(xtradata *data, PyObject *args) {
  ...
  int i = 0, j = 0;
  int n, nap, Nnonzero,
      Nzero;  // n is number of pointss, nap is number of access points
  ...
  if (!iterx) return -2;  // error not object

  PyObject *itery = PyObject_GetIter(objy);
  if (!itery) return -2;  // error not object

  PyObject *iterz = PyObject_GetIter(objz);
  if (!iterz) return -2;  // error not object
  ...
  nap = j;
  data->nap = nap;
  // if(debug){fprintf(stderr, "Number of access points: %d\n",nap);}
  if (i != 0) {
    return -5;
  }  // error size of x and z do not match
  ...
  for (j = 0; j < nap; ++j) {
    ...
    data->Nzero[j] = Nzero;
    if (debug) {
      fprintf(stderr, "%4d: Nonzero %5d \tZero %5d\n", j, Nnonzero, Nzero);
    }
  }
  ...
  fprintf(stderr, "Transfer data time %.7g ms.\n", elapsedTime);
...
void irls(double *p, int m, int n, void *data) {
  ...
  coarse_opts[0] = LM_INIT_MU;
  coarse_opts[1] = 1E-10;
  coarse_opts[2] = 1E-10;
  coarse_opts[3] = 1E-10;

  /* optimization control parameters; passing to levmar NULL instead of opts
  reverts to defaults double opts[LM_OPTS_SZ]; opts[0]=LM_INIT_MU;
  opts[1]=1E-15; opts[2]=1E-15; opts[3]=1E-20; opts[4]=LM_DIFF_DELTA; //
  relevant only if the finite difference Jacobian version is used
  */

  // Optimization with p[0], p[1] fixed
  ret = dlevmar_der(modelfun, jacmodelfunp23, p, NULL, m, n, 200, coarse_opts,
                    info, NULL, NULL, (void *)data);
  if (ret != -1) {
    if (debug) {
      fprintf(stderr, 
          "  [%.9f] -> [%.9f] Aftern Levenberg-Marquardt (%g iter, reason %g, "
          "yxy %.9f)\n",
  ...
  ret = dlevmar_der(modelfun, jacmodelfunp01, p, NULL, m, n, 50, coarse_opts,
                    info, NULL, NULL, (void *)data);
  if (ret != -1) {
    if (debug) {
      fprintf(stderr, 
          "  [%.9f] -> [%.9f] Aftern Levenberg-Marquardt (%g iter, reason "
          "%g)\n",
...
static PyObject *levmar_optimization(PyObject *self, PyObject *args) {
  ...
  if (load < 0) {
    fprintf(stderr, "Loading failed %d", load);
    return Py_BuildValue("n", load);
  }
  ...
  for (i = 0; i < nap; i++) {
    ...
    if (debug) {
      fprintf(stderr, "For access point %d\n", i);
    }
    while (convergence == 0) {
      iter++;
      ...
      // solve_linear(p,data); if(debug){fprintf(stderr, "\t%.3f %.3f %.3f %.3f (After
      // reweighting)\n", p[0], p[1], p[2], p[3]);} check convergence
      if (fabs(p[0] - p0_prev) < .05 && fabs(p[1] - p1_prev) < .005) {
        convergence = 1;
        if (debug) {
          fprintf(stderr, "\t%.3f %.3f %.3f %.3f (At convergence)\n", p[0],
                  p[1], p[2], p[3]);
        }
        ...
        if (debug) {
          fprintf(stderr, "%6d: %6d iter  %9.3f\n", i, iter, elapsedTime);
        }
      }
      p0_prev = p[0];
      p1_prev = p[1];
    ...
    fprintf(stderr, "Total time  %8.3f\n", elapsedTime);
  }
  ...
  if (!result) return Py_BuildValue("n", 6);  // PyTuple_New error
...
static PyMethodDef LevmarMethods[] = {
    {"optimize", levmar_optimization, METH_VARARGS,
     "Optimize parameters using levmar"},
    {NULL, NULL, 0, NULL}  // Sentinel
};

/* Initialization function */
static struct PyModuleDef levmar_module = {
    PyModuleDef_HEAD_INIT, "levmar", /* name of module */
    "",                              /* module documentation, may be NULL */
    -1, /* size of per-interpreter state of the module, or -1 if the module
           keeps state in global variables. */
    LevmarMethods};

PyMODINIT_FUNC PyInit_levmar(void) { return PyModule_Create(&levmar_module); }
...
int main(int argc, char *argv[]) {
  ...
  if (PyImport_AppendInittab("levmar", PyInit_levmar) == -1) {
  ...
  /* Optionally import the module; alternatively,
     ...
     imports it. */
  PyObject *pmodule = PyImport_ImportModule("levmar");
  if (!pmodule) {
    PyErr_Print();
    fprintf(stderr, "Error: could not import module 'levmar'\n");
  }

  PyMem_RawFree(program);
  return 0;

  //     Py_SetProgramName(argv[0]);     // Pass argv[0] to the Python
  //     interpreter Py_Initialize();    // Initialize Python interpreter
  //     PyObject *pmodule = PyImport_ImportModule("levmars");
  //     if (!pmodule) {
  //         PyErr_Print();
  //         ffprintf(stderr, stderr, "Error: could not import module 'spam'\n");
  //     }
  //
  //     PyMem_RawFree(argv[0]);
  //     //PyInit_levmars();       // Add static module
